...
%% Dynamics
K = 10;
f_free = [v;-g;-g] + [0;0;K*q(2) + u;-K*q(2) - u];
f_impact = [0;0;lzsq^2;0];
...
sos_4 = V; %subs(V,t,T);  % fixed end time
sos_5 = W;
...
[prog, sos_1, sig{end+1}, coefsig{end+1}] = spotless_add_sprocedure(prog, sos_1, phi, [V_vars;u], const_deg);
...
% Ball constraints
h_B = 1 - ([q;v])'*A*([q;v]);
h_BT = 1 - ([q;v]-xf)'*AT*([q;v]-xf);

[prog, sos_1, sig{end+1}, coefsig{end+1}] = spotless_add_sprocedure(prog, sos_1, h_B, [V_vars;u], const_deg);
...
% input constraints
[prog, sos_1, sig{end+1}, coefsig{end+1}] = spotless_add_sprocedure(prog, sos_1, 9 - u^2, [V_vars;u], const_deg);
...
close all
Vsol = sol.eval(V)
[X,Z] = meshgrid(linspace(-.1*R_diag(1),R_diag(1),100),linspace(-R_diag(2),R_diag(2),100));
