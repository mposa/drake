function [p,xtraj,utraj,ltraj,ljltraj,z,F,info,traj_opt] = testColocatedTrajOpt(xtraj,utraj,ltraj,ljltraj)
warning('off','Drake:RigidBodyManipulator:UnsupportedContactPoints');
warning('off','Drake:RigidBodyManipulator:WeldedLinkInd');
options.terrain = RigidBodyFlatTerrain();
options.floating = true;
options.ignore_self_collisions = true;
p = PlanarRigidBodyManipulator('KneedCompassGait_nolimits.urdf',options);
...
%   traj_init.lm = PPTrajectory(foh(t_init,repmat([.5;zeros(2,1)],2,N)));
%   traj_init.lp = PPTrajectory(foh(t_init,repmat([.5;zeros(2,1)],2,N)));  
%   traj_init.L = PPTrajectory(foh(t_init,repmat([zeros(3,1)],2,N)));
else
