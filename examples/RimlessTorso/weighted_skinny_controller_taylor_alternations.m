% having trouble with iteration 1 here on the weighted guy
% ran iterations 1,2,3 with no cost, but regularization
% obj was ?, 2.6e-3, 1.4e-5, 6.8e-4, 1.23e-5, 3e-4
megaclear
for iter = 7:7
...
file_prefix = 'weighted_zscale_skinny_cubic_controller_taylor_iter_%d';
...
elseif even(iter)
  options.regularize_eps = 5e-4; %cubic, have been using 1e-6. linear, iter 2, needed 1e-5
else
  options.regularize_eps = 5e-4; % 1e-4 failed for iter 1
end


%1e-6,1e-5 failed iter 2 after 1e-3 for iter 1


%% Taylor Dynamics
taylor_degree = 3;
taylor_vars = [q;qd;u;lx];
load weighted_skinny_taylor_eom
...
if iter==0,
  ...
  options.reg_cost = true;
  options.reg_var = bo_eps;
  cost = bo_eps;
%   cost = 0;
  if sos_option == 1
    rho_i = .01;
...
if do_backoff && ~isnumeric(cost) && ~options.reg_cost
