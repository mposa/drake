...
file_prefix = 'torso_taylor_iter_%d';
...
% odd iterations can be solved to higher accuracy,
if iter == 0,
...
if iter==0,
  cost = 0;
  rho_i = .03;  %worked (.02,2), (.03,2).  failed (.1,1), (.1,1), (.05,1)
  Ai = Ao2;
  Ao = Ao2;
  rho_o = 2;
  rho_Vo = 1;
...
use_additional_eqs = true;  % not a good idea here, probably not elsewhere either. vdot ~=0 at 0, pretty much always.
...
if even(iter)
  ...
  save(datapath(strcat(sprintf(file_prefix,iter))),'Vsol','R','AO','AI','D','sos1_mult','sos2_mult','sos3_mult','sos4_mult','sos5_mult')
else
