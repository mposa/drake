...
Vdot_free = detH*diff(V,[x;z;s;c;s_th;c_th])*[qd(1:2);c*pitchd;-s*pitchd;c_th*thetad;-s_th*thetad] + diff(V,qd)*adjH*(-C + B*u);
Vdot_impact_1 = diff(V,qd)*adjH*(J(1,:)'*lzsq(1) + J_f(1,:)'*lx(1));
Vdot_impact_2 = diff(V,qd)*adjH*(J(2,:)'*lzsq(2) + J_f(2,:)'*lx(2));
