function [x_opt, f_opt] = solveQPCC(Q,R,f_Q,f_R,H,C,B,J,Jf,phi0,Dphi,psi0,Dpsi,mu,u_lb,u_ub,q0,qd0,dt)
  ...
  function QP = generateQP(CCSpec)
    ...
    for j=1:nT,
      ...
      QP.A((j-1)*nIneqsi + i_free,(j-1)*nVarsi + (1:nQ)) = -Dphi{j}(i_free,:);  % phi0 + Dphi*q >= 0
      QP.b((j-1)*nIneqsi + i_free) = phi0{j}(i_free);
      
      % sticking contact
      i_stick = find(CCSpec{j} == 1);
      QP.B((j-1)*nEqnsi + 2*nQ + i_stick,(j-1)*nVarsi + (1:nQ)) = Dphi{j}(i_stick,:);  % phi0 + Dphi*q = 0
      ...
      QP.B((j-1)*nEqnsi + 2*nQ + i_slidingp,(j-1)*nVarsi + (1:nQ)) = Dphi{j}(i_slidingp,:);  % phi0 + Dphi*q = 0
      ...
      QP.B((j-1)*nEqnsi + 2*nQ + 3*nC + i_slidingp,(j-1)*nVarsi + 2*nQ + nU + i_slidingp) = mu*eye(length(i_stick));  % mulz - lx+ - lx- = 0;
      QP.B((j-1)*nEqnsi + 2*nQ + 3*nC + i_slidingp,(j-1)*nVarsi + 2*nQ + nU + nC + i_slidingp) = -eye(length(i_stick));
      QP.B((j-1)*nEqnsi + 2*nQ + 3*nC + i_slidingp,(j-1)*nVarsi + 2*nQ + nU + 2*nC + i_slidingp) = -eye(length(i_stick));
      
      % sliding psi < 0
      i_slidingm = find(CCSpec{j} ==  3);
      QP.B((j-1)*nEqnsi + 2*nQ + i_slidingm,(j-1)*nVarsi + (1:nQ)) = Dphi{j}(i_slidingm,:);  % phi0 + Dphi*q = 0
      ...
      QP.B((j-1)*nEqnsi + 2*nQ + 3*nC + i_slidingm,(j-1)*nVarsi + 2*nQ + nU + i_slidingm) = mu*eye(length(i_stick));  % mulz - lx+ - lx- = 0;
      QP.B((j-1)*nEqnsi + 2*nQ + 3*nC + i_slidingm,(j-1)*nVarsi + 2*nQ + nU + nC + i_slidingm) = -eye(length(i_stick));
      QP.B((j-1)*nEqnsi + 2*nQ + 3*nC + i_slidingm,(j-1)*nVarsi + 2*nQ + nU + 2*nC + i_slidingm) = -eye(length(i_stick));
        ...
        switch(parentQP.CCSpec{j}(k))
          case 0
            ...
            if abs(phi) < 1e-6
              ...
              psi = psi0{j} + Dpsi{j}(k,:)*x(1:2*nQ,j);
              if psi > 1e-6
                CCSpec_new{j}(k) = 2;
                if ~isVisited(CCSpec_new)
                  childQPs = [childQPs; generateQP(CCSpec_new)];
