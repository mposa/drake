...
%         dyn_inds{i} = [obj.h_inds(i);obj.x_inds(:,i);obj.x_inds(:,i+1);obj.u_inds(:,i);obj.l_inds(:,i);obj.ljl_inds(:,i)];
     ...
     function [f,df] = dynamics_constraint_fun(obj,h,x0,x1,u0,u1,lambda,lambda_jl)
        nq = obj.plant.getNumPositions;
        nv = obj.plant.getNumVelocities;
        nu = obj.plant.getNumInputs;
        nl = length(lambda);
        ...
        [H0,C0,B0,dH0,dC0,dB0] = obj.plant.manipulatorDynamics(q0,v0);
        [H1,C1,B1,dH1,dC1,dB1] = obj.plant.manipulatorDynamics(q1,v1); 
        
        BuminusC0 = B0*u0-C0; 
        BuminusC1 = B1*u1-C1; 
        dBuminusC0 = matGradMult(dB0,u0) - dC0;
        dBuminusC1 = matGradMult(dB1,u1) - dC1;
        
        [phi0,~,~,~,~,~,~,~,n0,D0,dn0,dD0] = obj.plant.contactConstraints(q0,false,struct('terrain_only',false));
        [phi1,~,~,~,~,~,~,~,n1,D1,dn1,dD1] = obj.plant.contactConstraints(q1,false,struct('terrain_only',false));
        % construct J and dJ from n,D,dn, and dD so they relate to the
        % lambda vector
        J0 = zeros(nl,nq);
        J0(1:2+obj.nD:end,:) = n0;
        dJ0 = zeros(nl*nq,nq);
        dJ0(1:2+obj.nD:end,:) = dn0;
        
        for j=1:length(D0),
          J0(1+j:2+obj.nD:end,:) = D0{j};
          dJ0(1+j:2+obj.nD:end,:) = dD0{j};
        end
        
        J1 = zeros(nl,nq);
        J1(1:2+obj.nD:end,:) = n1;
        dJ1 = zeros(nl*nq,nq);
        dJ1(1:2+obj.nD:end,:) = dn1;
        
        for j=1:length(D1),
          J1(1+j:2+obj.nD:end,:) = D1{j};
          dJ1(1+j:2+obj.nD:end,:) = dD1{j};
        end
        
        [~,J_jl0] = jointLimitConstraints(obj.plant,q0);
        [~,J_jl1] = jointLimitConstraints(obj.plant,q1);
     ...
     function [xdot,dxdot] = constrained_dynamics(obj,q,v,u,lambda,lambda_jl)
       nq = obj.plant.getNumPositions;
       nv = obj.plant.getNumVelocities;
       nu = obj.plant.getNumInputs;
       nl = length(lambda);
       ...
       [phi,~,~,~,~,~,~,~,n,D,dn,dD] = obj.plant.contactConstraints(q,false,struct('terrain_only',false));
       J = zeros(nl,nq);
       J(1:2+obj.nD:end,:) = n;
       dJ = zeros(nl*nq,nq);
       dJ(1:2+obj.nD:end,:) = dn;
       ...
       xdot = inv(H)*(B*u - C + J'*lambda + J_jl'*lambda_jl);
       % RHS = everything but the inv(H) above
       dRHSdx = matGradMult(dB,u) - dC + [matGradMult(dJ,lambda,true) + matGradMult(dJ_jl,lambda_jl,true);zeros(nq,nv)];
       dLHSdx = [-inv(H)*
     end
