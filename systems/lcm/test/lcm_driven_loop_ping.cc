...
const int kEnd = 10;
// Use default URL
const char kLcmUrl[] = "udpm://239.255.76.67:7667";
...
// A dummy test system that outputs the input message's time stamp in seconds.
class DummyPubSys : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(DummyPubSys);
  DummyPubSys() {
    DeclareAbstractOutputPort(&DummyPubSys::Output);
    DeclareVectorInputPort(systems::BasicVector<double>(1));
  }
  void Output(const Context<double>& context,
              lcmt_drake_signal* msg) const {
    const auto vector = this->EvalVectorInput(context, 0);
    msg->timestamp = vector->GetAtIndex(0);
  }
};
...
void publish() {
  ...
  for (int i = kStart; i <= kEnd; i++) {
    ...
    lcm.publish("ping", &msg);
    usleep(kSleepMicroSec);
  }
}

// The receiving Diagram in this test consists of a LcmSubscriberSystem,
// a DummySys and a LcmPublisherSystem. The intended behavior is that every 
// time a new Lcm message arrives, an empty message will be published back out
GTEST_TEST(LcmDrivenLoopTest, TestLoop) {
  ...
  auto dummy_pub = builder.AddSystem<DummyPubSys>();
  dummy_pub->set_name("dummy_pub");

  double pub_rate = 0;
  auto lcm_pub = builder.AddSystem(
    LcmPublisherSystem::Make<lcmt_drake_signal>("pong",
    &lcm, pub_rate));

  builder.Connect(*sub, *dummy);
  builder.Connect(*dummy, *dummy_pub);
  builder.Connect(*dummy_pub, *lcm_pub);
